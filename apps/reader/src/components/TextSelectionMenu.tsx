...
// import { translate, get_languages } from '../hooks/remote/translate_api'
...
import { currentLanguage } from './pages/settings'
...
// let language_pairs: any

// function get_code_from_name(name: string) {
//   const language = language_pairs.find((lang: any) => lang['name'] === name)
//   if (!language) {
...
const TextSelectionMenuRenderer: React.FC<TextSelectionMenuRendererProps> = ({
  ...
  // const setLanguagePairs = (lp: any) => {
  //   language_pairs = lp
  // }

  // useEffect(() => {
  //   const fetchLanguagePairs = async () => {
  //     const pairs = await get_languages(currentLanguage)
  ...
  const [_translate, setTranslate] = useState(false)
  const languages = language_pairs?.map((language: any) => language[0])
  const [targetLang, setTargetLang] = useState('fr')
  const [translatedText, setTranslatedText] = useState('')

  translate(text, currentLanguage, targetLang)
    .then((result) => {
      console.log('result')
      console.log(result)
      setTranslatedText(result as string)
      // Update state or do something else with translatedText if needed
    })
    .catch((error) => {
    ...
    <FocusLock disabled={mobile}>
      ...
      >
        {annotate ? (
          ...
          <div
            className={clsx(
              'typescale-body-small text-on-surface-variant placeholder:text-outline/60 flex-1 bg-transparent py-1 px-1.5 !text-[11px]',
              'scroll h-full resize-none',
              'max-w-[300px]',
            )}
          >
            <Fragment>{translatedText}</Fragment>
            <Select
              name={t('destination_language')}
              value={targetLang}
              onChange={(e) => {
            ...
            >
              {languages.map((lang: any) => (
                <option value={get_code_from_name(lang)}>{lang}</option>
              ))}
            </Select>
          </div>
        ) : (
